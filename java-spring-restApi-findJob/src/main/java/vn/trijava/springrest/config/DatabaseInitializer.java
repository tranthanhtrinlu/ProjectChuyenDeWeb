...
//import vn.trijava.springrest.repository.PermissionRepository;
//import vn.trijava.springrest.repository.RoleRepository;
//import vn.trijava.springrest.repository.UserRepository;
//import vn.trijava.springrest.util.constant.GenderEnum;
//
//@Service
...
public class DatabaseInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        if (countPermissions == 0) {
            ...
            permissions.add(new Permission("Download a file", "/api/v1/files", "GET", "FILES")); // Sửa từ POST thành GET
            permissions.add(new Permission("Upload a file", "/api/v1/files", "POST", "FILES")); // Sửa từ GET thành POST
        ...
        if (countUsers == 0) {
            ...
            adminUser.setGender(GenderEnum.MALE);
            adminUser.setName("I'm super admin");
            adminUser.setPassword(this.passwordEncoder.encode("Admin@123")); // Sử dụng mật khẩu mạnh hơn

            Role adminRole = this.roleRepository.findByName("SUPER_ADMIN");
            if (adminRole != null) {
                adminUser.setRole(adminRole);
            } else {
                throw new RuntimeException("Không tìm thấy role SUPER_ADMIN để gán cho admin user");
            }
