 ...
 */
export const callRegister = (name: string, email: string, password: string, age: number, gender: string, address: string) => {
    console.log("Sending register data:", { name, email, password, age, gender, address }); // Thêm log
    return axios.post<IBackendRes<IUser>>('/api/v1/auth/register', { name, email, password, age, gender, address });
}
...
export const callLogout = async () => {
    try {
        const response = await axios.post<IBackendRes<void>>('/api/v1/auth/logout');
        // Xóa access token từ localStorage
        localStorage.removeItem('access_token');
        // Xóa cookie refresh_token (nếu cần)
...
export const callForgotPassword = (email: string) => {
    console.log("Calling forgot password with email:", email); // Debug
    return axios.post<IBackendRes<string>>('/api/v1/auth/forgot-password', {}, { params: { email } });
};

export const callResetPassword = (resetCode: string, newPassword: string) => {
    return axios.post<IBackendRes<string>>('/api/v1/auth/reset-password', {}, { params: { resetCode, newPassword } });
}
